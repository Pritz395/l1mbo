"""Test in-memory Limbo server functionality via FastMCPTransport."""
import json
import sys
import pytest
...
async def test_in_memory_basic_tools(tmp_path):
    """Test basic Limbo tools work via in-memory transport."""
...
async def test_in_memory_server_management(tmp_path):
    ...
    async with client:
        ...
        # Remove server
        result = await client.call_tool("limbo_remove_server", {
            "name": "test-server"
        })
