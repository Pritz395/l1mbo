"""Tests for Limbo configuration management."""
...
class TestLimboConfig:
    """Test LimboConfig functionality."""

    def test_config_defaults(self):
        """Test default configuration values."""
        # Remove LIMBO env vars that might be set in container
        env = os.environ.copy()
        if 'LIMBO_LOG_LEVEL' in env:
            del env['LIMBO_LOG_LEVEL']
        if 'LIMBO_CONFIG_PATH' in env:
            del env['LIMBO_CONFIG_PATH']

        with patch.dict('os.environ', env, clear=True):
            config = LimboConfig()
            # config_path is now None by default, use get_config_path() for actual path
            assert config.config_path is None
            assert config.get_config_path() == Path.cwd() / ".limbo" / "config.json"
            ...
            assert config.path[0] == Path.cwd() / ".limbo"
            assert config.path[1] == Path.home() / ".limbo"
    ...
    def test_get_script_paths(self, tmp_path):
        ...
        # Test with custom LIMBO_PATH
        with patch.dict(os.environ, {"LIMBO_PATH": str(test_dir)}):
            config = LimboConfig()
