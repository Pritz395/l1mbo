"""Tests for Limbo authentication."""
import json
import tempfile
...
from limbo.auth import BearerAuthManager
from limbo.settings import AuthConfig, BearerAuthConfig, ConfigManager
...
class TestBearerAuthConfig:
    ...
    def test_defaults(self):
        ...
        assert config.issuer == "https://limbo.local"
        assert config.audience == "limbo"
        assert config.key_path == Path.home() / ".ssh" / "limbo"
...
class TestConfigManagerAuth:
    ...
    def test_load_auth_config_missing(self, tmp_path):
        ...
        assert auth_config.bearer.issuer == "https://limbo.local"
        assert auth_config.bearer.audience == "limbo"
    ...
    def test_save_load_auth_config(self, tmp_path):
        ...
        assert data == {
            "bearer": {
                "issuer": "https://test.example.com",
                "audience": "test-app",
                "key_path": str(Path.home() / ".ssh" / "limbo")
            }
        }
...
class TestBearerAuthManager:
    ...
    @patch.dict('os.environ', {'LIMBO_PRIVATE_KEY': ''})
    def test_load_private_key_from_env(self):
        ...
        with patch.dict('os.environ', {'LIMBO_PRIVATE_KEY': pem}):
