"""Tests for kit management tools in LimboServer."""
...
from limbo.server.response import LimboResponse
from limbo.kit import KitConfig
from limbo.settings import ServerConfig


class TestKitTools:
    """Test kit management tools in LimboServer."""

    @pytest_asyncio.fixture
    async def server(self, tmp_path):
        """Create a LimboServer instance with mocked components."""
    ...
    async def test_load_kit_already_loaded(self, server):
        ...
        from limbo.settings import KitInfo
...
class TestKitManagerWithServer:
    """Test kit manager initialization in LimboServer."""

    @pytest.mark.asyncio
    async def test_server_initializes_kit_manager(self, tmp_path):
        """Test that LimboServer properly initializes kit manager."""
        ...
        # Use LIMBO_PATH environment variable to set kit search path
        with patch.dict(os.environ, {"LIMBO_PATH": str(tmp_path)}):
            server = LimboServer(str(config_path), enable_config_reload=False)
    ...
    async def test_kit_tools_registered(self, tmp_path):
        ...
        assert "limbo_load_kit" in tools
        assert "limbo_unload_kit" in tools
        assert "limbo_list_kits" in tools
        assert "limbo_kit_info" in tools
