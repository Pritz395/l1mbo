...
class TestServerRemoval:
    ...
    @pytest.fixture
    def limbo_server(self, temp_config):
        """Create a Limbo server instance."""
        server = LimboServer(str(temp_config))
    ...
    @pytest.mark.asyncio
    async def test_remove_server_flow(self, limbo_server, config_manager):
        ...
        response = await limbo_server.remove_server("test1")
        ...
        list_response = await limbo_server.list_servers()
        ...
        response2 = await limbo_server.remove_server("test1")
    ...
    async def test_unmount_server_config_loading(self, config_manager):
        ...
        from limbo.server.manager import MountedServer
...
async def test_with_real_stdio_server(tmp_path):
    ...
    config_path = tmp_path / ".limbo" / "config.json"
