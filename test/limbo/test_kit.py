...
from limbo.kit import KitConfig, KitManager
from limbo.settings import ConfigManager, LimboConfig, ServerConfig
...
class TestKitManager:
    ...
    def test_limbo_path_integration(self, tmp_path):
        """Test that LIMBO_PATH environment variable works with kit manager."""
        ...
        # Test with LIMBO_PATH environment variable
        env_path = f"{custom_dir}:{another_dir}"
        with patch.dict(os.environ, {"LIMBO_PATH": env_path}):
            config_manager = ConfigManager()
            kit_manager = KitManager(config_manager)

            # Should find kit.d directories from LIMBO_PATH
