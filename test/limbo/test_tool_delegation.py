"""Test tool delegation functionality for Limbo."""
...
class TestToolDiscovery:
    ...
    async def test_server_tool_listing(self):
        ...
        async with Client(server.mcp) as client:
            ...
            # Should at least have Limbo's own tools
    ...
    async def test_mounted_server_tools(self):
        ...
        async with Client(server.mcp) as client:
            ...
            limbo_tools = [t for t in tool_names if t.startswith(server_prefix)]
            test_tools = [t for t in tool_names if t.startswith("test_")]

            assert len(limbo_tools) >= 2  # At least limbo_list_servers and limbo_add_server
