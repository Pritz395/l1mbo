...
from limbo.cli import cmd_kit
from limbo.kit import KitConfig
from limbo.settings import ServerConfig, LimboConfig
...
class TestKitCLI:
    ...
    async def test_kit_list(self, mock_args, mock_kit_manager, capsys):
        ...
        with patch('limbo.cli.KitManager', return_value=mock_kit_manager):
            with patch('limbo.cli.ConfigManager'):
    ...
    async def test_kit_list_empty(self, mock_args, capsys):
        ...
        with patch('limbo.cli.KitManager', return_value=manager):
            with patch('limbo.cli.ConfigManager'):
                await cmd_kit(mock_args)
    ...
    async def test_kit_load_success(self, mock_args, mock_kit_manager, capsys):
        ...
        with patch('limbo.cli.KitManager', return_value=mock_kit_manager):
            with patch('limbo.cli.ConfigManager', return_value=mock_config_instance):
                await cmd_kit(mock_args)
