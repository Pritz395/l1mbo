...
Examples:
  connect limbo "http://localhost:8080"
  connect calc "npx @wrtnlabs/calculator-mcp"
  tools
  call limbo_status
  call limbo_search_tools {"query": "calculator", "limit": 3}
  call add a=5 b=3
  search calculator
  info tool limbo_status
...
Quick Start:
  1. `connect limbo limbo serve` (connect to limbo server in stdio mode)
  2. `call limbo_<TAB>`  (shows available tools)
