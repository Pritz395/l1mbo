"""Server runner utilities for Limbo with proper signal handling.
"""
import asyncio
import logging
import signal
...
class LimboRunner:
    """Manages Limbo server lifecycle with proper signal handling."""
    _server: LimboServer
    ...
    def __init__(
        ...
        *,
        ...
        self._server = LimboServer(config_path, env=env)
    ...
    def client(self) -> Client:
        """Create an in-memory client connected to the Limbo server. [cached]"""
        return Client(FastMCPTransport(self._server.mcp))

    @property
    def server(self) -> LimboServer:
        """Get the current Limbo server instance."""
        return self._server
    ...
    async def run_hybrid(self, host: str = "localhost", port: int = 8000):
        ...
        try:
            async with self._server_context() as server:
                logger.debug("Starting Limbo hybrid server (stdio + HTTP on %s:%s)", host, port)
                await self._serve(server.run_hybrid(host, port))

        finally:
            logger.debug("Limbo hybrid server stopped")
