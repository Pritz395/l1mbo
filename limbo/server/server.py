"""Limbo - MCP Aggregator Server - Main Implementation
"""
import asyncio
import json
import logging
...
from .defaults import LIMBO_ADD_SERVER_DOC, PROXY_TOOL_DOC
from .manager import ManagedServer, ServerManager
from .response import LimboResponse
...
class LimboServer(ManagedServer):
    """Main Limbo server with tools for managing other MCP servers.
    """
    def __init__(
            self,
    ...
    def _register_tools(self):
        """Register all Limbo management tools programmatically.
        """
        ...
        def call_tool_wrapper(func):
            ...
            async def wrapper(*args, **kwds):
                ...
                if isinstance(result, LimboResponse):
...
"""
    ...
    async def add_server(
        ...
        try:
            ...
            if name in config.servers:
                return LimboResponse.error(f"Server '{name}' already exists")
            ...
            if cwd:
                ...
                if error:
                    return LimboResponse.error(error)
                cwd = validated_dir
            ...
            try:
                ...
                return LimboResponse.error(str(e))
            ...
            if server.enabled:
                ...
                if not mount_success:
                    return LimboResponse.error(f"Failed to mount server '{name}'")

            config.add_server(server)

            if not self.save_config(config):
                return LimboResponse.error(f"Failed to save configuration for added server '{name}'")

            return LimboResponse.success({
                "action": "server_added",
                "server": {
                    "name": server.name,
            ...
            return LimboResponse.error(f"Failed to add server: {str(e)}")
    ...
    async def remove_server(
        ...
        try:
            ...
            if name in config.servers:
                ...
                if not self.save_config(config):
                    return LimboResponse.error(f"Failed to save configuration after removing server '{name}'")
                ...
                return LimboResponse.error(f"Server '{name}' not found")

        except Exception as e:
            return LimboResponse.error(f"Failed to remove server: {str(e)}")

    async def list_servers(self) -> LimboResponse:
        ...
        try:
            ...
            return LimboResponse.success(servers)

        except Exception as e:
            return LimboResponse.error(f"Failed to list servers: {str(e)}")
    ...
    async def enable_server(
        ...
        try:
            ...
            if server.enabled:
                return LimboResponse.error(f"Server '{name}' is already enabled")

            server.enabled = True

            if not self.save_config(config):
                return LimboResponse.error(f"Failed to save configuration for server '{name}'")
            ...
            return LimboResponse.error(f"Failed to enable server: {str(e)}")
    ...
    async def disable_server(
        ...
        try:
            ...
            if not server.enabled:
                return LimboResponse.error(f"Server '{name}' is already disabled")
            ...
            return LimboResponse.error(f"Failed to disable server: {str(e)}")
    ...
    async def smart_configure(
        ...
        try:
            ...
            prompt = f"""You are being asked by the Limbo smart_configure tool to analyze metadata and generate an optimal MCP server configuration.
...
=== TASK ===
Based on the URL and metadata above, generate a complete JSON configuration that will be automatically added to the user's Limbo server configuration.
* Search for existing tools in the MCP catalog if needed
* Search the web for additional setup instructions if needed
* Examine and use the Limbo MCP server tools directly as needed.
  - The (un-prefixed) `proxy` tool can be used to call any MCP capability and interact with Limbo's dynamic state.
...
"""
            ...
            if allow_add:
                if not result or not result.text:
                    return LimboResponse.error("Failed to get configuration from LLM")

                try:
                    json_match = re.search(r'{.*}', result.text, re.DOTALL)
                    if not json_match:
                        return LimboResponse.error("No valid JSON configuration found in LLM response")
                    ...
                    return LimboResponse.error(f"Failed to parse LLM configuration: {str(e)}")
            ...
            return LimboResponse.error(f"Smart configuration failed: {str(e)}")
    ...
    async def search_servers(
        ...
        try:
            ...
            return LimboResponse.error(f"Failed to search servers: {str(e)}")
    ...
    async def analyze_servers(
        ...
        try:
            ...
            if ctx:
                prompt = f"""Analyze this Limbo server configuration and provide insights:
...
4. Missing capabilities that could be added"""
            ...
            return LimboResponse.success(analysis_data)

        except Exception as e:
            return LimboResponse.error(f"Failed to analyze servers: {str(e)}")

    async def status(self) -> LimboResponse:
        """Get basic Limbo server status and statistics."""
        try:
            config = self.config
            ...
            return LimboResponse.success(status_data)

        except Exception as e:
            return LimboResponse.error(f"Failed to get status: {str(e)}")

    async def reload_config_tool(self) -> LimboResponse:
        ...
        if not self.config.auto_reload:
            return LimboResponse.error(
                "Configuration reload is disabled. Set LIMBO_AUTO_RELOAD=true to enable."
            )
        ...
        try:
            ...
            if success:
                ...
                return LimboResponse.error("Configuration reload failed - check logs for details")

        except Exception as e:
            logger.exception("Error during config reload")
            return LimboResponse.error(f"Config reload error: {str(e)}")
    ...
    async def check(
        ...
        try:
            ...
            return LimboResponse.error(f"Failed to check servers: {str(e)}")
