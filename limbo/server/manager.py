"""Server management for Limbo - mounting, unmounting, and tracking MCP servers.
"""
import asyncio
import logging
import os
...
from .defaults import LIMBO_INSTRUCTIONS
...
from ..settings import ConfigManager, LimboConfig, ServerConfig
...
class ServerManager:
    ...
    def __init__(self, config_manager: ConfigManager, *, env: dict | None = None):
        ...
        self.mcp = ProxyFastMCP(
            ...
            instructions=LIMBO_INSTRUCTIONS.format(self_prefix=self.self_prefix),
            auth=auth_provider
        )
        self.mounted_servers = {}

    @property
    def config(self) -> LimboConfig:
        """Get the current Limbo configuration."""
        return self.config_manager.load_config()

    def save_config(self, config: LimboConfig):
        """Save the current configuration to disk."""
        return self.config_manager.save_config(config)

    @cached_property
    def prefix_separator(self) -> str:
        """Get the prefix separator for this Limbo server - cannot be changed during process lifetime."""
        return self.config.prefix_sep

    @cached_property
    def self_prefix(self) -> str:
        """Get the self prefix for this Limbo server - cannot be changed during process lifetime."""
...
class ManagedServer:
    ...
    @cached_property
    def self_prefix(self) -> str:
        """Get the self prefix for this Limbo server.
    ...
    async def setup(self):
        """Initialize Limbo and mount existing servers.
    ...
    async def run_hybrid(self, host: str = "localhost", port: int = 8000, log_level: str | None = None):
        """Run Limbo in hybrid mode - both stdio and HTTP simultaneously.
        """
        log_level = log_level or os.getenv("FASTMCP_LOG_LEVEL", "CRITICAL").upper() or "CRITICAL"
        ...
        try:
            ...
            if success:
                if not self.save_config(config):
                    return LimboResponse.error("Failed to save configuration")
                ...
                return LimboResponse.error(message)

        except Exception as e:
            return LimboResponse.error(f"Failed to load kit: {str(e)}")
    ...
    async def unload_kit(
        ...
        try:
            ...
            return LimboResponse.error(f"Failed to unload kit: {str(e)}")

    async def list_kits(self) -> LimboResponse:
        ...
        try:
            ...
            return LimboResponse.error(f"Failed to list kits: {str(e)}")
    ...
    async def kit_info(
        ...
        try:
            ...
            if info:
                return LimboResponse.success(info)
            else:
                return LimboResponse.error(f"Kit '{name}' not found")

        except Exception as e:
            return LimboResponse.error(f"Failed to get kit info: {str(e)}")
