"""Configuration management for Limbo - Using pydantic-settings."""
import json
import logging
import os
...
__all__ = "ServerConfig", "LimboConfig", "ConfigManager", "AuthConfig", "BearerAuthConfig", "ClientSettings", "KitInfo"
...
class ClientSettings(BaseSettings):
    ...
    jwt: str | None = Field(
        default=None,
        description="JWT token for authentication (env: LIMBO_JWT)"
    )
...
class BearerAuthConfig(BaseSettings):
    ...
    issuer: str = Field(default="https://limbo.local", description="Token issuer identifier")
    audience: str = Field(default="limbo", description="Token audience")
    key_path: Path = Field(
        default_factory=lambda: Path.home() / ".ssh" / "limbo",
        description="Path for private & public key storage"
    )

    @property
    def private_key_env(self) -> str | None:
        """Get private key from LIMBO_PRIVATE_KEY environment variable."""
        return os.environ.get('LIMBO_PRIVATE_KEY')
...
class LimboConfig(BaseSettings):
    """Main Limbo configuration."""
    model_config = SettingsConfigDict(
        env_prefix="LIMBO_",
        env_file=".env",
        extra="allow",
    ...
    path: str | list[Path] = Field(
        default_factory=lambda: [
            get_project_root() / ".limbo",
            Path.home() / ".limbo",
            *get_contrib_paths()
        ],
        description="Multi-path search list for config.json and kit.d directories (env: LIMBO_PATH, colon-separated)"
    )
    config_path: Path | None = Field(
        default=None,
        description="Explicit configuration file path (overrides path search) (env: LIMBO_CONFIG_PATH)"
    )
    read_only: bool = Field(default=False, description="Run in read-only mode (env: LIMBO_READ_ONLY)")
    log_level: str | None = Field(default=None, description="Logging level for Limbo (default: INFO) (env: LIMBO_LOG_LEVEL)")
    self_prefix: str = Field(
        default="limbo",
        description="Prefix for Limbo tools and commands - must be a valid Python identifier without underscores (env: LIMBO_SELF_PREFIX)"
    )
    prefix_sep: str = Field(
        default="_",
        description="Separator between prefix and tool name (env: LIMBO_PREFIX_SEP)"
    )
    auto_reload: bool = Field(default=True, description="Enable automatic config reloading on file changes (env: LIMBO_AUTO_RELOAD)")
    reload_poll_interval: float = Field(default=1.0, description="Config file poll interval in seconds (env: LIMBO_RELOAD_POLL_INTERVAL)")
    stderr_show: bool = Field(default=False, description="Show stderr output from subprocess MCP servers (env: LIMBO_STDERR_SHOW)")
    ...
    def parse_path(cls, v) -> list[Path]:
        """Parse LIMBO_PATH environment variable or return default."""
        if isinstance(v, str):
    ...
    @model_validator(mode='after')
    def export_environment_variables(self) -> 'LimboConfig':
        """Export configuration as environment variables for child processes."""
        if 'LIMBO_LOG_LEVEL' not in os.environ:
            os.environ['LIMBO_LOG_LEVEL'] = self.log_level or 'INFO'

        if self.config_path and 'LIMBO_CONFIG_PATH' not in os.environ:
            os.environ['LIMBO_CONFIG_PATH'] = str(self.config_path)

        if 'LIMBO_AUTO_RELOAD' not in os.environ:
            os.environ['LIMBO_AUTO_RELOAD'] = str(self.auto_reload).lower()

        if 'LIMBO_READ_ONLY' not in os.environ:
            os.environ['LIMBO_READ_ONLY'] = str(self.read_only).lower()
...
class ConfigManager:
    """Manages Limbo configuration persistence."""
    config_path: Path
    ...
    def load_config(self) -> LimboConfig:
    ...
    def save_config(self, config: LimboConfig) -> bool:
