"""Transport utilities for Limbo - handles FastMCP transport selection and configuration.
"""
import shlex
import sys
...
def get_transport_for_input(connect_string: str) -> ClientTransport:
    ...
    Examples:
        >>> transport = get_transport_for_input("python -m limbo serve")
...
def parse_command_string(command_string: str) -> tuple[str, list[str]]:
    ...
    Args:
        command_string: Command string like "python -m limbo serve" or "npx @playwright/mcp@latest"

    Returns:
        Tuple of (command, args) where command is the first word and args is the rest

    Examples:
        >>> parse_command_string("python -m limbo serve")
        ("python", ["-m", "limbo", "serve"])
...
def get_transport_for_command_string(
    ...
    Args:
        command_string: Full command as string (e.g., "python -m limbo serve")
    ...
    Examples:
        >>> transport = get_transport_for_command_string("python -m limbo serve")
