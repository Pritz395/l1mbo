"""Main CLI interface for Limbo - Simplified implementation."""
...
from .server.runner import LimboRunner
...
async def cmd_serve(args) -> int:
    ...
    runner = LimboRunner(args.config, env=env)
...
async def cmd_disable_server(args) -> int:
    ...
    if config_manager.save_config(config):
        ...
        print_text("If Limbo is running, the server will be automatically unmounted")
        return 0
    else:
        print_error("Failed to save configuration")
        return 1
        ...
        case 'status':
            ...
            if auth_config.bearer.private_key_exists:
                ...
                if auth_config.bearer.private_key_env:
                    print_info("Private key also available via LIMBO_PRIVATE_KEY env var")
                ...
                print_text("Run 'limbo auth init' to enable authentication")
            return 0

        case 'token':
            auth_config = config_manager.load_auth_config()
            if not auth_config.bearer.private_key_exists:
                print_error("No authentication keys found. Run 'limbo auth init' first")
                return 1
            ...
            try:
                ...
                print_error(str(e))
                print_info("Run 'limbo auth init' to generate keys")
                return 1
            ...
            if args.quiet:
                ...
                print(f"export LIMBO_JWT={token}")
            else:
            ...
            if args.auth_action == 'public-key':
                ...
                if private_key:
                    ...
                    if args.export:
                        ...
                        print(f"export LIMBO_PRIVATE_KEY={single_line}")
                    elif args.oneline:
                        single_line = pem.replace('\n', '\\n')
                        print(single_line)
                    else:
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog='limbo',
        description='Limbo - MCP Aggregator: Manage and aggregate MCP servers',
        epilog='Use "limbo <command> --help" for more information about a command.'
    )
    ...
    parser.add_argument(
        '--config',
        type=str,
        help='Path to config file (default: .limbo/config.json in current directory)'
    )
    ...
    serve_parser = subparsers.add_parser(
        'serve',
        help='Start Limbo server',
        description='Start the Limbo server in either stdio mode (default) or HTTP mode'
    )
    ...
    auth_init.add_argument('--issuer', help='Token issuer identifier (default: https://limbo.local)')
    auth_init.add_argument('--audience', help='Token audience, also used as key name (default: limbo)')
    auth_init.add_argument('--key-path', type=Path, help='Path for authentication keys (default: ~/.ssh/limbo)')
